...
def run_train():
    ...
    for name, param in model.named_parameters():
        if args.freeze and 'deit' in args.backbone:
            if name.startswith('module.blocks') and int(name[14])<args.freeze:
                param.requires_grad = False
        # if not 'local_head' in name and not 'Reranker' in name:
        #     param.requires_grad = False
    ...
    with torch.cuda.device(0):
        macs, params = get_model_complexity_info(model.module, (3, args.resize[0], args.resize[1]), as_strings=False,
                                                 print_per_layer_stat=True, verbose=True)
        logging.info(f"flops:{macs/1e9}, params: {params/1e6}")
    ...
    for epoch_num in range(start_epoch_num, args.epochs_num):
        ...
        util.save_checkpoint(args, {"epoch_num": epoch_num, "model_state_dict": model.state_dict(),
            "optimizer_state_dict": optimizer.state_dict(), "recalls": recalls, "best_r5": best_r1,
